...
DATA_FILES = {
    ...
    "paraphrased": ROOT / "data" / "paraphrases" / "paraphrased_200.json",
...
class ModelInference:
    ...
    def extract_answer(self, decoded: str) -> Optional[str]:
        tail = decoded.split("Answer:")[-1].strip()
        match = re.search(r"\b([ABCD])\b", tail.upper())
        if match:
            return match.group(1)
        for char in ["A", "B", "C", "D"]:
            if char in tail.upper():
                return char
        return None

    def predict(self, question: str, choices: Sequence[str]) -> Optional[str]:
...
class PerturbationSensitivityDetector:
    ...
    def score_example(self, example: Dict[str, object], perturbations: List[Dict[str, object]]) -> ExampleResult:
        ...
        original_prediction = self.inference.predict(question, choices)
        original_correct = self.is_correct(original_prediction, answer_idx)

        details: List[Dict[str, object]] = []
        pert_correct: List[int] = []
        for perturbation in perturbations:
            pred = self.inference.predict(str(perturbation["question"]), perturbation["choices"])
        ...
        sensitivity = float(original_correct - pert_avg)
...
def optimize_ensemble(
    ...
    test_auc, _, test_tpr, test_fpr = youden_threshold(labels[test_idx], test_scores)
    return {
...
def evaluate_condition(
    ...
    prefix_auc, prefix_threshold, prefix_tpr, prefix_fpr = youden_threshold(labels, prefix_all)
    pert_auc, pert_threshold, pert_tpr, pert_fpr = youden_threshold(labels, pert_all)
    ensemble = optimize_ensemble(prefix_all, pert_all, labels, seed=SEED)

    prefix_ci = bootstrap_metric(labels, prefix_all, prefix_threshold, rng)
    pert_ci = bootstrap_metric(labels, pert_all, pert_threshold, rng)
    ...
    val_idx = ensemble["validation_idx"]
    test_idx = ensemble["test_idx"]

    prefix_norm = normalize(prefix_all)
    pert_norm = normalize(pert_all)
    prefix_test_scores = prefix_norm[test_idx]
    pert_test_scores = pert_norm[test_idx]
...
def main() -> None:
    ...
    condition_results = []
    for model_key in ["model_v", "model_p"]:
        for contamination_split in ["verbatim", "paraphrased"]:
